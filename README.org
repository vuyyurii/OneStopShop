...
#+NAME: steps
#+BEGIN_SRC
Go to the app directory.Next You have to create database tables for storing information required by our app.
This can be done by entering python interpreter.Type python in terminal an press enter.
Then in python interpreter execute command =from app import db=,then execute command =db.create_all()=.
Now our required database tables are created.
